...
import { Lock, Mail, LogIn, ShieldCheck, User } from 'lucide-react';
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (res.data.role !== 'customer') {
                alert("Access denied. This login is for customers only.");
                return;
            }
            ...
            localStorage.setItem('userName', res.data.name || "Customer");
            
            navigate(`/dashboard/${res.data.role}`);
        } catch (err) {
            alert("Login failed. Check your user credentials.");
        }
    };
            ...
            <div className="bg-white rounded-2xl shadow-2xl overflow-hidden max-w-md w-full p-8">
                <div className="text-center mb-10">
                    <div className="bg-pink-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4" style={{backgroundColor: '#bc9d6e'}}>
                        <LogIn className="text-white" size={32} style={{color: 'white'}} />
                    </div>
                    <h1 className="text-3xl font-extrabold text-gray-900">User Login</h1>
                    <p className="text-gray-500 mt-2">Sign in to manage your AI Helpdesk system</p>
                </div>
                ...
                <form onSubmit={handleLogin} className="space-y-6">
                    <div className="relative">
                        ...
                        <input 
                            type="email" placeholder="Customer Email Address" required
                    ...
                    <div className="relative">
                        ...
                        <input 
                            type="password" placeholder="Customer Password" required
                            className="w-full pl-11 pr-4 py-3 border border-gray-300 rounded-xl outline-none transition-all" style={{backgroundColor: 'rgba(255, 255, 255, 0.9)', color: '#569296'}}
                ...
                <div className="mt-8 pt-6 border-t border-gray-100 text-center">
                    <p className="text-gray-600 text-sm">
                        ...
                        <Link to="/register" className="ml-2 font-bold hover:underline" style={{color: '#bc9d6e'}}>
                            Create one now
                        </Link>
                    </p>
                </div>
